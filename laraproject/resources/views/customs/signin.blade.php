...
<?php
 ...
 $manakeyword='xhome_xhome2_xproduct_cart_xxxxx';
 if(isset($_GET['manakeyword'])){	$manakeyword=$_GET['manakeyword'];		}
 //$manakeyword='home_home2_cart';
...
<table class="tb_creat03"align="center"border="0">
  ...
  <form method="POST" id="contactform" action="/customs/signin2">
    @csrf

  <tr><td align="center"colspan="4" class="td_creat05"><strong>Sign in</strong></td></tr>
<tr><td class="td_creat05a"></td><td colspan="2">
  <input type="email" class="login5"name="email" value="{{ \Cookie::get('custemail') }}  "placeholder=" email"/>
  @error('email')
    <div class="div_creat06">{{ $message }}</div>
  @enderror
...
<tr><td></td><td colspan="2">
    ...
    <input class="login5" type="password"id="password"name="password"value="{{ \Cookie::get('custpass') }}" placeholder="Password" style="border-radius:5px;color:#333;border:none;background-color:#fff; height:40px;width:99%">
    @error('password')
      <div class="div_creat06">{{ $message }}</div>
    @enderror
...
<input type="hidden" name="manakeyword" value="<?php echo $manakeyword; ?>" checked>
...
<tr><td align="center"><a href="{{ url('customs/creat0') }}" class="a_creat08">Creat an acount</a></br>
