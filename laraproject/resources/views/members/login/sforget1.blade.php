...
<tr><td align="center"colspan="4" ></td></tr>
  <form method="POST" id="contactform" action="/members/sforget2">
       @csrf
<tr><td align="center"colspan="4" class="td_creat05"><strong>Set new password...</strong></td></tr>
<tr><td class="td_creat05a"></td><td colspan="2">
  @if(isset( $arr12 ))<p style="font-size:10px;color:#fff "></p>
    <input type="text" name="email" value="{{$arr12[1]}}"placeholder=" email"class="in_creat09">
    @error('email')
      <div class="div_creat06">{{ $message }}</div>
    @enderror

   @else
<input type="email" name="email" value="{{ old('email') }}" placeholder=" email"class="in_creat09"/>
  ...
  <tr><td></td><td colspan="2">
      <input type="text"name="code2z"value="{{ old('code2z') }}" class="in_creat09"placeholder="Code confirm" >
      @error('code2z')
        <div class="div_creat06">{{ $message }}</div>
      @enderror
    </td><td> </td></tr>
...
<tr><td align="center">
<a href="{{ url('members/sforget0') }}"class="a_creat08"><h3> Back </h3></a>
</td></tr></table>
